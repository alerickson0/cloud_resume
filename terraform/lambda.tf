  ...
  policy = jsonencode({
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Resource": [
                "arn:aws:logs:${local.current_region}:${local.account_id}:log-group:/aws/lambda/updateVisitorCounter:*"
            ]
        }
    ]
  })
...
resource "aws_lambda_function" "updateVisitorCounter_lambda" {
  filename         = "${path.module}/back-end/lambda/updateVisitorCounter.zip)"
  function_name    = "updateVisitorCounter_lambda"
  role             = aws_iam_role.updateVisitorCounter-role-h7zmdlm8.arn

  source_code_hash = filebase64sha256("${path.module}/back-end/lambda/updateVisitorCounter.zip")
